...
pub async fn handle_lock<B: Backend>(
    ...
    let fid_state = session
        .fids
        .get(fid)
        .ok_or_else(|| unknown_fid(fid, "Tlock"))?;
    ...
    let status = tokio::task::spawn_blocking(move || {
        ctx.backend
            .lock(&handle, lock_type, flags, start, length, proc_id)
    })
    .await
    .map_err(join_err)??;
    ...
    Ok(Fcall {
        size: 0,
        msg_type: MsgType::Rlock,
        tag,
        msg: Msg::Rlock { status },
    })
}
...
pub async fn handle_getlock<B: Backend>(
    ...
    let fid_state = session
        .fids
        .get(fid)
        .ok_or_else(|| unknown_fid(fid, "Tgetlock"))?;
    let handle = fid_state
        .handle
        .as_ref()
    ...
    let result = tokio::task::spawn_blocking(move || {
        ctx.backend
            .getlock(&handle, lock_type, start, length, proc_id)
    })
    .await
    .map_err(join_err)??;
