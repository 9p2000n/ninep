    ...
    match fc.msg_type {
        MsgType::Tstreamopen => {
            ...
            let fid_state = session
                .fids
                .get(fid)
                .ok_or_else(|| unknown_fid(fid, "Tstreamopen"))?;
            ...
            Ok(Fcall {
                size: 0,
                msg_type: MsgType::Rstreamopen,
                tag,
                msg: Msg::Rstreamopen { stream_id },
            })
        }
        MsgType::Tstreamdata => {
            let Msg::Streamdata {
            ...
            tracing::trace!(
                ...
                seq,
                len = data.len(),
                "Tstreamdata received"
            );

            let stream = session.active_streams.get(&stream_id).ok_or_else(|| {
            ...
            if direction == STREAM_WRITE {
                ...
                let ctx = ctx.clone();
                let written =
                    tokio::task::spawn_blocking(move || ctx.backend.write(&handle, offset, &data))
                        .await
                        .map_err(join_err)??;
            ...
            let removed = if let Some((_, stream)) = session.active_streams.remove(&stream_id) {
                ...
                if stream.direction == STREAM_WRITE {
                    ...
                    match tokio::task::spawn_blocking(move || ctx.backend.fsync(&handle)).await {
                        Ok(Ok(())) => {}
                        Ok(Err(e)) => {
                            tracing::warn!(stream_id, error = %e, "Tstreamclose: fsync failed")
                        }
                        Err(e) => {
                            tracing::warn!(stream_id, error = %e, "Tstreamclose: fsync task panicked")
                        }
                    }
                }
                true
