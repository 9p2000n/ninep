...
pub async fn handle<B: Backend>(
    ...
    let fid_state = session
        .fids
        .get(fid)
        .ok_or_else(|| unknown_fid(fid, "Thash"))?;
    ...
    let ctx = ctx.clone();
    let hash = tokio::task::spawn_blocking(move || ctx.backend.hash(&handle, algo, offset, length))
        .await
        .map_err(join_err)??;
    ...
    Ok(Fcall {
        size: 0,
        msg_type: MsgType::Rhash,
        tag,
        msg: Msg::Rhash { algo, hash },
    })
}
